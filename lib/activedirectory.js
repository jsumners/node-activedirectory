...
ActiveDirectory.prototype.findOrganizationalUnits = function findOrganizationalUnits(opts, callback) {
  var self = this
  var defaultOrganizationalUnitFilter = '(&(ou=*))'
  var localOpts = _.defaults(_.omit(opts || {}, 'attributes'), {
    filter: '(&'+defaultOrganizationalUnitFilter+')',
    scope: 'sub',
    attributes: joinAttributes((opts || {}).attributes || [])
  });
  searcher.call(self, localOpts, function onSearch(err, results) {
    if (err) {
      if (callback) callback(err)
      return;
    }
    if ((! results) || (results.length === 0)) {
      log.warn('No OUs found matching query "%s"', truncateLogOutput(localOpts.filter));
